...
| XML_PATH               | Path to the xml files inside docker file                          |
...
- XML_FILE should be stored to `etl/data.xml` otherwise you need to change docker compose file. 
- PDM wasn't used because it's a matter of personal preference. However, in real projects, using PDM/Poetry, and so on,
  is not difficult
